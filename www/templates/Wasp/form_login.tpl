...
[case:[special:lang]|
  ...
  [equ:helpLogin=
    ...
    <p>If the "Persistent login" checkbox is not checked (default), Serverswebhosting uses so called "session cookies" that are automatically removed when you close your browser.</p>
...
|
  ...
  [equ:helpLogin=
    ...
    <p>Если флажок «Постоянный логин» не установлен (по умолчанию), Serverswebhosting использует так называемые «сеансовые куки», которые автоматически удаляются при закрытии браузера.</p>
...
|
  ...
  [equ:helpLogin=
    ...
    <p>Si la case "Resté connecté" n'est pas cochée, Serverswebhosting utilisera un cookie de sessions qui sera automatiquement détruit lorsque vous quitterez le site.</p>
...
|
  ...
  [equ:helpLogin=
    ...
    <p>Ist es nicht gesetzt (das ist der Standard), so verwendet Serverswebhosting so genannte "Sitzungscookies", die automatisch gelöscht werden, wenn Sie Ihren Browser schließen.</p>
